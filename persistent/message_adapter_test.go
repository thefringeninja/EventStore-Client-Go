...
func Test_MessageAdapter(t *testing.T) {
	...
	require.Equal(t, expectedMessage.CreatedDate.String(), message.OriginalEvent().CreatedDate.String())
	// because it is hard to compare time in tests set this to be equal so that we can compare other fields
	expectedMessage.CreatedDate = message.OriginalEvent().CreatedDate

	require.Equal(t, expectedMessage, message.OriginalEvent())
}
